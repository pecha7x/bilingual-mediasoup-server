...
module.exports = class Room {
  ...
  getProducerListForPeer(socket_id) {
    // we get all others Peers from the room and then build their producerList
    let producerList = [];
  ...
  async startRecording(socket_id) {
    if (!this.peers.has(socket_id)) return;

    const peer = this.peers.get(socket_id);
    peer.startRecording(this.router);
  }

  async stopRecording(socket_id) {
    if (!this.peers.has(socket_id)) return;
