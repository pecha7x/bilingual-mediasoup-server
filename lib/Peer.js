...
const { uploadToS3 } = require('./recording/S3FileUploader');
...
module.exports = class Peer {
  ...
  async startRecordingGstreamer(cmdInputPath) {
    ...
    const pathToResultFile = `${__dirname}/recording/results/${this.room_id}`;
    fs.mkdirSync(pathToResultFile, { recursive: true });
    this.recOutputPath = `${pathToResultFile}/${this.name}.webm`;
